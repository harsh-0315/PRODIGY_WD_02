...
function addLap() {
    if (!isRunning && interval) {
        ...
        lapDiv.innerHTML = `<h3>Lap ${count}</h3> <p>${pad(minutes)}:${pad(seconds)}:${pad(Math.floor(milliseconds / 10))}</p> <button class="deleteBtn"><i class="ri-close-line"></i></button>`;
...
function deleteLap(e) {
    const del = e.target.parentElement.previousElementSibling;
    parentElem = del.previousElementSibling.parentElement;
...
lapRecords.addEventListener("click", (e) => {
